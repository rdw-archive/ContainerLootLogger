...
function Detection.OnUnitSpellcastSucceeded(...)
	...
	if unit == "player" and trackedSpells[spellID] then -- Player cast a spell that will lead to them looting stuff
		...
		local secs = 1.4 -- TODO: If this delay is longer than the shortest cast time (e.g., 1.5sec for Salvage), then a new cast can finish while tracking is still in progress/locked, which means the 2nd cast will either mess up the first one's results, or go (partially) undetected
